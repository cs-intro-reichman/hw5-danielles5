public class Wordle {
    ...
    public static String[] readDictionary(String filename) {
        ...
        while (!readFile.isEmpty()) {
            String word = readFile.readString();
            wordCount++;
        }

        // Creates an array to hold all the words in the file
        String[] words = new String[wordCount];

        // Puts each word in its own 'box' in the array
        In theWord = new In(filename);
        int i = 0;

        while (!theWord.isEmpty()) {
            words[i] = theWord.readString();
            i++;
        }
    ...
    public static String chooseSecretWord(String[] dict) {
        ...
        // gives a random location of our chosen word
        int locationRandomNum = (int)(Math.random() * length);

        // finds the String, our chosen word
        String theWord = dict[locationRandomNum];
    ...
    public static boolean containsChar(String secret, char c) {
        ...
        // Checks if each character in the String is equal to c
        ...
        // If no letter is equal, then c does not appear in secret
        return false;
    }
    ...
    public static boolean isAllGreen(char[] resultRow) {
        ...
        for (int i = 0; i < resultRow.length; i++) {
            if (resultRow[i] == 'G') {
                // nothing, keep going through all letters
            } else {
                return false;
            }
        }
